<script lang="ts">
    ...
    export let graphNodes: Array<{
        id: string;
        type: string;
        label?: string;
        weight?: number;
    }> = [];
    export let graphEdges: Array<{
        from: string;
        to: string;
        relation: string;
    }> = [];
            ...
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-4 flex-1">
                    ...
                    <div class="flex items-center gap-2 min-w-[140px]">
                        ...
                        <span
                            class="text-sm {vadState.isSpeaking
                                ? 'text-green-400'
                                : 'text-slate-400'} font-medium flex items-center gap-1.5"
                        >
                            <Icon
                                name={vadState.isSpeaking
                                    ? "microphone"
                                    : "listening"}
                                size={14}
                            />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div
                        class="w-24 h-2 bg-dark-900/80 rounded-full overflow-hidden"
                    >
                        <div
                ...
                <div class="flex items-center gap-4 text-xs text-slate-500">
                    ...
                    <span class="text-slate-400"
                        >{(vadState.vadConfidence * 100).toFixed(0)}%</span
                    >
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-8">
                <div class="glass-card p-4 h-full border-cyan-500/20">
                    <div class="flex items-center justify-between mb-3">
                        <span
                            class="text-xs text-slate-400 uppercase tracking-wider font-bold"
                            >Live Transcript</span
                        >
                        {#if isGeminiConnected}
                            <span
                                class="badge-success text-xs flex items-center gap-1"
                                ><Icon name="cpu" size={12} /> AI Active</span
                            >
                        {/if}
                    </div>
                    ...
                    <div
                        class="transcript-scroll space-y-3 max-h-[350px] overflow-y-auto pr-2"
                    >
                                ...
                                >
                                    <div class="max-w-[85%]">
                                        ...
                                        >
                                            ...
                                            {#if t.tone}
                                                <span
                                                    class="px-1.5 py-0.5 text-[9px] rounded bg-cyan-500/20 text-cyan-400 uppercase"
                                                    >{t.tone}</span
                                                >
                                            {/if}
                                        </div>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {#if t.isPartial}
                                                    ...
                                                    >
                                                        <span
                                                            class="w-1 h-1 bg-cyan-500/50 rounded-full animate-bounce"
                                                        ></span>
                                                        <span
                                                            class="w-1 h-1 bg-cyan-500/50 rounded-full animate-bounce"
            ...
            <div class="col-span-4 space-y-3">
                ...
                <div class="glass-card p-3 border-cyan-500/20">
                    ...
                    <div
                        class="h-24 relative overflow-hidden bg-dark-900/40 rounded-lg"
                    >
                        ...
                        {:else}
                            <div
                                class="flex items-center justify-center h-full text-slate-600 text-xs"
                            >
                                Topics appear here
                            </div>
                        {/if}
                ...
                <div class="glass-card p-3 border-cyan-500/20">
                    <span
                        class="text-xs text-slate-400 uppercase tracking-wider block mb-2"
                        >Psychosomatic Engine</span
                    >
                    <div class="space-y-2">
                        <!-- Stress -->
                        <div class="flex items-center gap-2">
                            <span
                                class="text-[9px] text-slate-500 w-16 uppercase"
                                >Stress</span
                            >
                            <div
                                class="flex-1 h-1.5 bg-dark-600 rounded-full overflow-hidden"
                            >
                                <div
                                    class="h-full transition-all duration-500"
                                    style="width: {stressLevel *
                                        100}%; background: {getGaugeColor(
                                        stressLevel,
                                    )};"
                                ></div>
                            </div>
                            <span
                                class="text-[9px] w-8 text-right"
                                style="color: {getGaugeColor(stressLevel)}"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-[9px] text-cyan-400 w-8 text-right"
                                >{(engagementLevel * 100).toFixed(0)}%</span
                            >
                        </div>
                        <!-- Urgency -->
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-[9px] w-8 text-right"
                                style="color: {getGaugeColor(urgencyLevel)}"
                                >{(urgencyLevel * 100).toFixed(0)}%</span
                            >
                        </div>
                        <!-- Clarity -->
                        <div class="flex items-center gap-2">
                            ...
                            >
                                <div
                                    ...
                                    style="width: {clarityLevel * 100}%;"
                                ></div>
                            </div>
                            <span
                                class="text-[9px] text-green-400 w-8 text-right"
                                >{(clarityLevel * 100).toFixed(0)}%</span
                            >
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card p-3 border-cyan-500/20">
                    <div class="flex items-center gap-3">
                        <div class="relative">
                            <div
                                class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center {isGeminiConnected
                                    ? 'animate-pulse'
                                    : ''}"
                            >
                                <Icon
                            ...
                            {#if isGeminiConnected}
                                <div
                                    class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 rounded-full border border-dark-700"
                                ></div>
                            {/if}
                        </div>
                        <div class="flex-1">
