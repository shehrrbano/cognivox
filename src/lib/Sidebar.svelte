<script lang="ts">
    ...
    function onSessionLoad(session: any) {
        dispatch("sessionLoad", session);
    }
</script>
...
<div class="w-72 sidebar flex flex-col">
    ...
    <div class="p-4 flex-1">
        ...
        <div class="sidebar-card h-48 overflow-hidden">
            <KnowledgeGraph
                ...
                edges={graphEdges}
                compact={true}
                on:toggleCluster={(e) => dispatch("toggleCluster", e.detail)}
            />
        </div>
    </div>
    ...
    <div class="p-4 border-t border-cyan-500/10">
        <SessionManager {currentSession} {onSessionLoad} />
    </div>
</div>
