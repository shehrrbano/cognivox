...
<script lang="ts">
    ...
    let riskNodes = $derived((graphNodes || []).filter((n: GraphNode) =>
        n.type === 'RISK' || n.type === 'Risk' ||
        (n.id && n.id.toUpperCase().includes('RISK'))
    ));

    let totalRisks = $derived(riskNodes.length);
    let highSeverityRisks = $derived(riskNodes.filter((n: GraphNode) => (n.weight || 0) >= 0.7).length);
    ...
    let timeline = $derived.by(() => {
        ...
        return (pastSessions || []).slice(0, 5).map((s: any, i: number) => {
    ...
    let risks = $derived.by(() => {
        ...
        return riskNodes.slice(0, 3).map((n: GraphNode) => ({
            title: n.label || n.id,
    ...
    let tracker = $derived.by(() => {
        if (riskNodes.length === 0) return [];
        return riskNodes.slice(0, 3).map((n: GraphNode, i: number) => ({
            title: n.label || n.id,
    ...
    {#if showToast}
        <div class="fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-[#0f172a] text-white px-5 py-3 rounded-lg shadow-2xl flex items-center justify-between min-w-[360px] z-50 animate-fadeIn">
            ...
            <div class="flex items-center gap-4">
                <button class="text-blue-600 font-bold text-[11px] hover:text-blue-400 transition-colors uppercase tracking-wider">UNDO</button>
                <button class="text-gray-400 hover:text-white" onclick={() => showToast = false} aria-label="Close notification">
    ...
    <div class="max-w-6xl mx-auto space-y-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            <div>
                ...
                <p class="text-[14px] text-gray-500 font-medium">Real-time risk mitigation and deadline tracking.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-5">
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)] h-[280px] flex flex-col">
                <div class="flex justify-between items-center mb-5">
                    ...
                    <button class="text-[11px] font-bold text-blue-600 hover:underline">View All</button>
                </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)]">
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {#if (tracker || []).length === 0}
                            <tr><td colspan="5" class="py-8 text-center text-[11px] text-gray-400 font-medium">No mitigation items.</td></tr>
                        {/if}
                        {#each (tracker || []) as tr}
                            <tr class="border-b border-gray-50">
                                ...
                                <td class="py-4 px-2 w-[120px]"><span class="text-[11px] font-medium text-gray-600">{tr.domain}</span></td>
                                <td class="py-4 px-2 w-[100px]"><div class="flex items-center gap-1.5"><div class={`w-2 h-2 rounded-full ${tr.sevDot}`}></div><span class={`text-[10px] font-bold ${tr.sevText}`}>{tr.sev}</span></div></td>
                                <td class="py-4 px-4 w-[160px]"><div class="flex items-center justify-center gap-2"><div class={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black tracking-tighter ${tr.user.col}`}>{tr.user.initials}</div><span class="text-[11px] font-semibold text-gray-700">{tr.user.label}</span></div></td>
                                <td class="py-4 px-4 w-[120px]"><span class={`px-2.5 py-1 text-[9px] font-black uppercase tracking-wider rounded border bg-opacity-70 ${tr.statusBg}`}>{tr.status}</span></td>
                            </tr>
                        {/each}
                    </tbody>
                </table>
